...
export default function NavbarUser() {
  ...
  return (
    <Suspense>
      <nav className="fixed top-0 left-0 w-full z-50 bg-white dark:bg-gray-800 shadow-lg text-black dark:text-gray-100">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-700 dark:text-gray-100 rounded-lg md:hidden"
          ...
          <div className={`${isMobileMenuOpen ? 'block' : 'hidden'} w-full md:flex md:w-auto items-center gap-4`} id="navbar-dropdown">
            <ul className="flex flex-col md:flex-row items-start md:items-center md:space-x-15 mt-4 md:mt-0 font-medium">
              <li>
                <Link href="/" className="block py-2 px-3 text-black dark:text-gray-100 hover:text-[#996515] md:p-0">
                  Home
                </Link>
              </li>
              <li>
                <Link href="/artikel" className="block py-2 px-3 text-black dark:text-gray-100 hover:text-[#996515] md:p-0">
                  Berita
                </Link>
              </li>
              ...
              <li>
                ...
                <Link href="/layanan" className="block md:hidden py-2 px-3 hover:text-[#996515]">
                  Profil
                </Link>
              </li>
      ...
      <section className={`fixed flex justify-center z-50 top-18 py-4 w-full bg-white dark:bg-gray-800 shadow-lg transition-all duration-200 ${isNavbarOpen ? 'opacity-100 pointer-events-auto visible' : 'opacity-0 pointer-events-none invisible'}`} onMouseEnter={showDropdown} onMouseLeave={scheduleHide}>
        <div className="flex flex-wrap justify-center gap-8">
          {[
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center border dark:bg-gray-300 border-gray-300 rounded-sm">
                <Image src={`/icons/${layanan.icon}.svg`} alt={layanan.label} width={100} height={100} className="w-6 h-6" />
              </div>
              <span className="text-base font-semibold text-gray-700 dark:text-gray-100">{layanan.label}</span>
            </a>
          ))}
        </div>
      </section>
      <section className={`fixed flex justify-center z-50 top-18 py-4 w-full bg-white dark:bg-gray-800 shadow-lg transition-all duration-200 ${isProfilOpen ? 'opacity-100 pointer-events-auto visible' : 'opacity-0 pointer-events-none invisible'}`} onMouseEnter={showProfilDropdown} onMouseLeave={scheduleProfilHide}>
        <div className="flex flex-wrap justify-center gap-8">
          {[
            ...
            >
              <div className="w-12 h-12 flex items-center justify-center border dark:bg-gray-300 border-gray-300 rounded-sm">
                <Image src={`/icons/${profil.icon}.svg`} alt={profil.label} width={100} height={100} className="w-6 h-6" />
              </div>
              <span className="text-base font-semibold text-gray-700 dark:text-gray-100">{profil.label}</span>
            </a>
          ))}
        </div>
      </section>
